...
async def handle_release_command(message):
    ...
    try:
        ...
        amount_btc = data["amount"] / (await get_ltc_price_usd())
...
async def send_role_selection(channel):
    ...
    channel_data[channel.id]["role_msg"] = msg.id
...
async def update_role_selection(channel):
    ...
    role_msg_id = channel_data[channel.id].get("role_msg")
    if role_msg_id:
        role_msg = await channel.fetch_message(role_msg_id)
        await role_msg.edit(embed=embed, view=view)
    else:
        msg = await channel.send(embed=embed, view=view)
        channel_data[channel.id]["role_msg"] = msg.id
